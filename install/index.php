...
if($method == 'show_license') {
	...
	if($submit) {
		...
		$ret = false;
		if(is_dir('saestor://'.SAE_STORAGE_DOMAIN_NAME.'/data/backup')) {
			$ret = @rename('saestor://'.SAE_STORAGE_DOMAIN_NAME.'/data/backup', 'saestor://'.SAE_STORAGE_DOMAIN_NAME.'/data/backup_'.$backupdir);
		}
		if(!$ret) {
			@mkdir('saestor://'.SAE_STORAGE_DOMAIN_NAME.'/data/backup_'.$backupdir, 0777);
		}
		if(is_dir('saestor://'.SAE_STORAGE_DOMAIN_NAME.'/data/backup_'.$backupdir)) {
		...
		$db->query("REPLACE INTO {$tablepre}common_setting (skey, svalue) VALUES ('attachdir', 'saestor://discuzx/data/attachment/')");
$db->query("REPLACE INTO {$tablepre}common_setting (skey, svalue) VALUES ('attachurl', 'http://".$_SERVER['HTTP_APPNAME']."-".SAE_STORAGE_DOMAIN_NAME.".stor.sinaapp.com/data/attachment/')");

@file_put_contents('saestor://discuzx/crossdomain.xml',file_get_contents(ROOT_PATH.'crossdomain.xml'));
$db->query("ALTER TABLE `{$tablepre}home_album` CHANGE `pic` `pic` VARCHAR( 360 ) NOT NULL DEFAULT ''");
