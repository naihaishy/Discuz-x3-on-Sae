...
class base {
	...
	function init_input($getagent = '') {
		...
		if($input) {
			...
			//SAE/fetchurl-w1yyo2lx31 Mozilla/5.0 (Windows NT 5.1; rv:2.0.1) Gecko/20100101 Firefox/4.0.1
			$this->input = daddslashes($this->input, 1, TRUE);
			$_SERVER['HTTP_USER_AGENT'] = preg_replace('/^sae\S+\s/i','',$_SERVER['HTTP_USER_AGENT']);
			$this->input['agent'] = preg_replace('/^sae\S+\s/i','',$this->input['agent']);
	...
	function &cache($cachefile) {
		...
		if(!isset($_CACHE[$cachefile])) {
			$cachepath = 'saekv://uc_server/data/cache/'.$cachefile.'.php';
