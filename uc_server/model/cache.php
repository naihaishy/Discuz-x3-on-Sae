	...
	function updatedata($cachefile = '') {
		if($cachefile) {
			foreach((array)$this->map[$cachefile] as $modules) {
				...
				$s .= "\r\n?>";
				@file_put_contents("saekv://uc_server/data/cache/$cachefile.php", $s);
			}
		} else {
			foreach((array)$this->map as $file => $modules) {
				...
				$s .= "\r\n?>";
				@file_put_contents("saekv://uc_server/data/cache/$file.php", $s);
			}
		}
	}

	function updatetpl() {clearkvfolder('/uc_server\/data\/view');
