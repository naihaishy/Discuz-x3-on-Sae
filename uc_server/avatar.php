...
define('UC_DATADIR', 'saestor://'.SAE_STORAGE_DOMAIN_NAME.'/uc_server/data/');
define('UC_DATAURL', 'http://'.$_SERVER['HTTP_APPNAME'].'-'.SAE_STORAGE_DOMAIN_NAME.'.stor.sinaapp.com/uc_server/data/');
...
if(file_exists(UC_DATADIR.$avatar)) {
	if($check) {
		echo 1;
		exit;
	}
...
header('Location: '.UC_DATAURL.$avatar_url);
exit;
