...
if($operation == 'patch' || $operation == 'cross') {
	...
	if($step == 1) {
		...
		if(!$_GET['startupgrade']) {
			...
			foreach($updatefilelist as $updatefile) {
				...
				$backfile = 'saekv://data/back/Discuz! X'.substr(DISCUZ_VERSION, 1).' Release['.DISCUZ_RELEASE.']/'.$updatefile;
				if(is_file($destfile)) {
		...
		foreach($updatefilelist as $updatefile) {
			$srcfile = 'saekv://data/update/Discuz! X'.$version.' Release['.$release.']/'.$updatefile;
			if($confirm == 'ftp') {
				$destfile = $updatefile;
			} else {
				$destfile = DISCUZ_ROOT.$updatefile;
		...
		if($upgradeinfo['isupdatedb']) {
			...
			foreach($dbupdatefilearr as $dbupdatefile) {
				$srcfile = 'saekv://data/update/Discuz! X'.$version.' Release['.$release.']/'.$dbupdatefile;
		...
		$file = 'saekv://data/update/Discuz! X'.$version.' Release['.$release.']/updatelist.tmp';
		@unlink($file);
	...
	$upgrade_step = C::t('common_cache')->fetch('upgrade_step');
	$upgrade_step = dunserialize($upgrade_step['cachevalue']);
	$file = 'saekv://data/update/Discuz! X'.$upgrade_step['version'].' Release['.$upgrade_step['release'].']/updatelist.tmp';
	@unlink($file);
	@unlink(DISCUZ_ROOT.'./install/update.php');
	C::t('common_cache')->delete('upgrade_step');
	C::t('common_cache')->delete('upgrade_run');
