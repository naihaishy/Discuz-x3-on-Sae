...
class image {
	...
	function init($method, $source, $target, $nosuffix = 0) {
		...
		if( substr($source,0,10) != 'saestor://' && isset($parse['host'])) {
			if(empty($target)) {
				return -2;
			}
			$data = dfsockopen($source);
			$this->tmpfile = $source = tempnam(SAE_TMP_PATH.'/temp/', 'tmpimg_');
	...
	function Thumb_GD() {
		...
		clearstatcache();
		if($thumb_photo) {
			$tmpfile1 = tempnam(SAE_TMP_PATH.'/temp/', 'tmpimg_');
			if($this->imginfo['mime'] == 'image/jpeg') {
				@$imagefunc($thumb_photo, $tmpfile1, $this->param['thumbquality']);
			} else {
				@$imagefunc($thumb_photo, $tmpfile1);
			}
			file_put_contents($this->target,file_get_contents($tmpfile1));
			return 1;
		} else {
			return 0;
		}
