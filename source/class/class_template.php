...
class template {
	...
	function parse_template($tplfile, $templateid, $tpldir, $file, $cachefile) {
		...
		if(file_exists('saekv://'.$tplfile)){
			$template = file_get_contents('saekv://'.$tplfile);
		}
		elseif(file_exists('saekv://'.$tpldir.$file.'.htm')){
            		$template = file_get_contents('saekv://'.$tpldir.$file.'.htm');
        	}
		elseif($fp = @fopen(DISCUZ_ROOT.$tplfile, 'r')) {
		...
		if(!@$fp = fopen('saekv://'.$cachefile, 'w')) {
			$this->error('directory_notfound', dirname('saekv://'.$cachefile));
		}
		...
		if(!isset($langvar[$var])) {
			...
			if(!$isplugin) {
				...
				if(empty($_G['config']['plugindeveloper'])) {
					...
					if(@include('saekv://data/plugindata/'.$vars[0].'.lang.php')) {
	...
	function loadcsstemplate() {
		...
		$content = @implode('', file('saekv://data/cache/style_'.STYLEID.'_module.css'));
		...
		if($this->csscurmodules) {
			...
			if(@$fp = fopen('saekv://data/cache/style_'.STYLEID.'_'.$_G['basescript'].'_'.CURMODULE.'.css', 'w')) {
