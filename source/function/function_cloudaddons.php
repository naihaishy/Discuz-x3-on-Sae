...
function cloudaddons_check() { return ;
	...
	foreach(array('download', 'addonmd5') as $path) {
		$tmpdir = 'saekv://data/'.$path.'/'.random(5);
...
function cloudaddons_getmd5($md5file) {
	$array = array();
	if(preg_match('/^[a-z0-9_\.]+$/i', $md5file) && file_exists('saekv://data/addonmd5/'.$md5file.'.xml')) {
		require_once libfile('class/xml');
		$xml = implode('', @file('saekv://data/addonmd5/'.$md5file.'.xml'));
		$array = xml2array($xml);
	} else {
		return false;
	}
...
function cloudaddons_uninstall($md5file, $dir) {
	...
	@unlink('saekv://data/addonmd5/'.$md5file.'.xml');
...
function cloudaddons_savemd5($md5file, $end, $md5) {
	...
	if(!isset($_G['siteftp'])) {
		dmkdir('saekv://data/addonmd5/', 0777, false);
		$fp = fopen('saekv://data/addonmd5/'.$md5file.'.xml', 'w');
		fwrite($fp, array2xml($array));
		fclose($fp);
	} else {
		$localfile = 'saekv://data/'.random(5);
...
function cloudaddons_comparetree($new, $old, $basedir, $md5file, $first = 0) {
	global $_G;
	if($first && file_exists('saekv://data/addonmd5/'.$md5file.'.xml')) {
		require_once libfile('class/xml');
		$xml = implode('', @file('saekv://data/addonmd5/'.$md5file.'.xml'));
		$array = xml2array($xml);
...
function cloudaddons_dirwriteable($basedir, $dir, $sourcedir) { return true;
	$checkdirs = array($dir);
