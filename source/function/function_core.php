...
function lang($file, $langvar = null, $vars = array(), $default = null) {
	...
	if($path != 'plugin') {
		...
		if(empty($_G['config']['plugindeveloper'])) {
			...
			if(@include('saekv://data/plugindata/'.$file.'.lang.php')) {
...
function template($file, $templateid = 0, $tpldir = '', $gettplfile = 0, $primaltpl='') {
	...
	$_G['style']['tpldirectory'] = str_replace('./','',$_G['style']['tpldirectory']);
	$_G['style']['prefile'] = '';
			$diypath = 'saekv://data/diy/'.$_G['style']['tpldirectory'].'/'; //DIY模板文件目录
			$preend = '_diy_preview';
	...
	if($templateid != 1 && !file_exists(DISCUZ_ROOT.$tplfile) && !file_exists('saekv://'.$tplfile) && !file_exists(substr(DISCUZ_ROOT.$tplfile, 0, -4).'.php')
	...
	$cachefile = str_replace('./','',$cachefile);
		checktplrefresh($tplfile, $tplfile, @filemtime('saekv://'.$cachefile), $templateid, $cachefile, $tpldir, $file);
	return 'saekv://'.$cachefile;
}
