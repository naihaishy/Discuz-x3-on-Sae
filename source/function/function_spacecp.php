...
function album_update_pic($albumid, $picid=0) {
	...
	$basedir = !getglobal('setting/attachdir') ? ('saekv://data/attachment/') : getglobal('setting/attachdir');
...
function videophoto_upload($FILE, $uid) {
	if($FILE['size']) {
		...
		if(!is_dir('saekv://data/avatar/'.$dir1)) {
			if(!mkdir('saekv://data/avatar/'.$dir1)) return '';
		}
		if(!is_dir('saekv://data/avatar/'.$dir1.'/'.$dir2)) {
			if(!mkdir('saekv://data/avatar/'.$dir1.'/'.$dir2)) return '';
		}
